...
test("sync persists resolved and unresolved Panel products with status counts", async () => {
  ...
  const fakeFetch = async (input: string | URL | Request) => {
    ...
    const data = [
      ...
      { ...common, id: "panel-comp-1", sku: "FABRIC", title: "Panel Kumaş", catalog_type: "complementary", catalog_version_ref: `catalog-product:panel-comp-1:v${version}`, base_uom: { code: "square_meter", base_quantum: "square_millimeter", quantity_scale: 1000000 }, material_behavior: "continuous_cut", mass_grams: 12 },
  ...
  assert.deepEqual((db.prepare("SELECT unit_type FROM complementary_products WHERE id LIKE 'panel-comp-%' ORDER BY base_uom_code").all() as Array<{ unit_type: string }>).map((row) => row.unit_type), ["box", "kg", "meter", "package", "piece", "roll", "square_meter"]);
  assert.equal((db.prepare("SELECT material_behavior FROM complementary_products WHERE id='panel-comp-1'").get() as any).material_behavior, "continuous_cut");
